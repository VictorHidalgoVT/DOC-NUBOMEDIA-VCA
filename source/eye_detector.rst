...
This web application  consists on a WebRTC video communication with a eye
...
application accesing the following URL: **localhost:8108**
...
The main class of this demo is named NuboEyeJavaApp. As you can see, the
...
requests in the path /nuboeyedetector.

NuboEyeJavaHandler class implements TextWebSocketHandler to handle text
...
the Server: start, show_eyes, scale_factor, process_num_frames,
...
(WebRtcEndpoint and NuboEyeDetectorFilter) and make the connections among them.
...
(variable ws) in the path /nuboeyedetector. Then, the onmessage listener of the
